...
CATEGORY_APPLIED_DISPLAY_COLUMNS = ['금융사', '계좌번호', '거래일', '거래시간', '입금액', '출금액', '취소', '기타거래', '키워드', '카테고리', '사업자번호', '폐업', '출처', '위험도키워드', '위험도분류', '위험도']
# cash_after 생성 시 저장 컬럼. 폐업 = '폐업' 또는 ''. 출처 = '은행거래'|'신용카드'(요약용)
CASH_AFTER_CREATION_COLUMNS = ['금융사', '계좌번호', '거래일', '거래시간', '입금액', '출금액', '취소', '기타거래', '키워드', '카테고리', '사업자번호', '폐업', '출처', '위험도키워드', '위험도분류', '위험도']
...
def index():
    ...
    CATEGORY_QUERY_분류_MYCASH = ('심야구분', '업종분류', '위험도분류')
    category_table_data = []
    try:
...
def print_analysis():
    ...
    try:
        ...
        # 위험도분류별 집계 (1호~10호) 및 인쇄용 표시명
        risk_classification_rows = _build_risk_classification_rows_from_df(df)
        ...
        # 리포트 작성: 금융정보 위험도 테이블 기반 변수 + 프롬프트와 동일한 Python 로직으로 R 산출
        current_status = [
        ...
        try:
            ...
            if df_cat is not None and not df_cat.empty and '분류' in df_cat.columns:
                신청인_rows = df_cat[df_cat['분류'].fillna('').astype(str).str.strip() == '신청인']
...
def _get_opinion_context(bank_filter=None, period=None):
    ...
    # ── cash_after 위험도 요약 (종합의견에 위험도 현황 포함, bank_filter·period 적용됨) ──
    try:
...
def analysis_risk_report():
    ...
    try:
        ...
        risk_report = _calculate_risk_report(RISK_GUIDELINES_FIXED, current_status)
        return render_template('risk_report_page.html', risk_report=risk_report, filter_institution=bank_filter or '')
    except Exception as e:
        traceback.print_exc()
