...
try:
    ...
    def _safe_취소(val):
        ...
        if DIRECTION_CANCELLED in s or CANCELLED_TRANSACTION in s: return DIRECTION_CANCELLED
        return s
...
from lib.category_table_io import (
    load_category_table, CATEGORY_TABLE_COLUMNS,
...
from lib.category_constants import (
    DIRECTION_CANCELLED, CANCELLED_TRANSACTION, get_template_constants,
...
def _remove_bad_data_file(path, recreate_empty=None):
    ...
    if p.exists() and p.stat().st_size > 0:
        try:
            ...
            if winerr == 32 or errno_val == 13:
                logger.warning("안내: %s이(가) 다른 프로그램에서 열려 있어 삭제할 수 없습니다. 파일을 닫은 뒤 다시 시도하세요.", p.name)
            elif winerr != 2 and errno_val != 2:
                logger.error("삭제 실패 %s: %s", p, ex)
    if recreate_empty is not None:
        try:
            ...
            logger.error("빈 데이터 파일 재생성 실패 %s: %s", p, ex)
...
def load_category_file():
    ...
    if _bank_after_cache_obj is None:
        try:
            ...
            logger.error("load_category_file 오류: %s", e)
            return pd.DataFrame()
    try:
...
def get_source_files():
    ...
    try:
        ...
        logger.exception("파일 목록 로드 중 오류")
        current_dir = os.getcwd()
        return jsonify({
    ...
    try:
        ...
        if _bank_after_cache_obj is None or _bank_after_cache_obj.current is None:
            ...
            try:
                ...
                logger.warning("ensure_bank_before_and_category 실패: %s", _e)
            finally:
                if _path_added and str(SCRIPT_DIR) in sys.path:
                    sys.path.remove(str(SCRIPT_DIR))
        ...
        try:
            ...
            logger.exception("카테고리 파일 로드 오류")
        ...
        if account_filter and '계좌번호' in df.columns:
            df = df[df['계좌번호'].fillna('').astype(str).str.strip() == account_filter].copy()
...
def category():
    """카테고리 페이지"""
    return render_template('category.html', **get_template_constants('bank'))
...
def get_category_table():
    ...
    try:
        ...
        try:
            ...
            logger.warning("ensure_bank_before_and_category/migrate 실패 (기존 파일 유지): %s", _e)
            if not path.exists():
                try:
        ...
        if file_existed and (df is None or df.empty) and path.exists() and path.stat().st_size > 0:
            ...
            try:
                ...
                _corrupted = True
            if _corrupted and str(path).endswith('.json'):
                logger.warning("category_table.json 손상 감지 → 빈 파일로 재생성")
                try:
                    path.unlink()
                    from lib.category_table_io import create_empty_category_table
                    create_empty_category_table(str(path))
        ...
        if str(path).endswith('.json') and not path.exists():
            try:
                from lib.category_table_io import create_empty_category_table
                create_empty_category_table(str(path))
            except Exception as _e:
                logger.warning("category_table 빈 파일 생성 실패: %s", _e)
                ...
                if not acct_df.empty and '카테고리' in acct_df.columns:
                    ...
                    for _, r in acct_df.iterrows():
                        ...
                        if cat and cat not in major_map:
                            major_map[cat] = maj
                    if major_map and category_col in df.columns:
                        df['_대분류'] = df[category_col].map(major_map).fillna('Ⅶ. 미분류')
                        ms = df.groupby('_대분류').agg({'입금액': 'sum', '출금액': 'sum'}).reset_index()
                        ms = ms.rename(columns={'_대분류': '대분류'})
                        ms['건수'] = df.groupby('_대분류').size().values
                        roman_order = {'Ⅰ': 1, 'Ⅱ': 2, 'Ⅲ': 3, 'Ⅳ': 4, 'Ⅴ': 5, 'Ⅵ': 6, 'Ⅶ': 7, 'Ⅷ': 8, 'Ⅸ': 9, 'Ⅹ': 10}
...
def generate_category():
    ...
    try:
        ...
        logger.exception("bank_after 생성 API 오류")
        return _json_500({
            'success': False,
            'error': f'{str(e)}\n상세 정보는 서버 로그를 확인하세요.'
        })
